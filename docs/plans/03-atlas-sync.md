...
- **P5 (→G2,G6, Atlas-side)** — make `_compute_sync_version` hash the **same** source `/sync` reads, and include the server URL in the hash; add a telemetry assertion the two tables agree while the bridge exists. *(Atlas repo change — flagged.)*
