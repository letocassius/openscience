...
rg -n "Instance.directory.*Instance.worktree|every matching file|system-loaded or message-loaded" ARCHITECTURE.md
git diff --check
```

Expected: `ARCHITECTURE.md` has no `CLAUDE.md` link; all four architecture subsections are present; instruction source order remains `AGENTS.md`, `CLAUDE.md`, `CONTEXT.md`; the directory-to-worktree all-match behavior and on-read exclusions are documented; `git diff --check` exits 0.
...
Expected: exit 0; with a session opened at the fixture root, instruction tests confirm its root `AGENTS.md` is already included in `systemPaths()` while a deeper `AGENTS.md` is discovered on read; the no-config default-agent test confirms `research`.
